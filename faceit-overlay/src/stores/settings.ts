...
const DEFAULT_SETTINGS: OverlaySettings = {
  ...
  cards: [
    ...
    { id: '5', type: 'avgKills', enabled: false, order: 4, duration: 10 },
    { id: '6', type: 'avgDeaths', enabled: false, order: 5, duration: 10 },
    { id: '7', type: 'recentMatches', enabled: false, order: 6, duration: 10 },
    { id: '8', type: 'eloDiff', enabled: false, order: 7, duration: 10 },
    { id: '9', type: 'winStreak', enabled: false, order: 8, duration: 10 },
    { id: '10', type: 'performance', enabled: false, order: 9, duration: 10 },
    { id: '11', type: 'maps', enabled: false, order: 10, duration: 10 },
    // Tier 1 новые карточки
    { id: '12', type: 'multiKill', enabled: false, order: 11, duration: 10 },
    { id: '13', type: 'mapPerformance', enabled: false, order: 12, duration: 10 },
    { id: '14', type: 'performanceTrend', enabled: false, order: 13, duration: 10 },
    // Tier 2 новые карточки
    { id: '15', type: 'entryFragging', enabled: false, order: 14, duration: 10 },
    { id: '16', type: 'mvpAssist', enabled: false, order: 15, duration: 10 },
    { id: '17', type: 'sessionStats', enabled: false, order: 16, duration: 10 }
  ],
  rotation: {
    enabled: true,
    interval: 10,
...
export const useSettingsStore = defineStore('settings', () => {
  ...
  const liveMatchUrl = computed(() => {
    const baseUrl = window.location.origin + '/live-match'
    const params = new URLSearchParams()
    ...
    if (settings.value.liveMatch.opacity !== 100) {
      params.append('opacity', settings.value.liveMatch.opacity.toString())
    }

    if (settings.value.liveMatch.updateInterval !== 30000) {
      params.append('updateInterval', settings.value.liveMatch.updateInterval.toString())
    }
  ...
  function loadFromUrl() {
    ...
    if (window.location.pathname === '/live-match') {
      const updateInterval = params.get('updateInterval')
      if (updateInterval) {
        const intervalValue = parseInt(updateInterval)
        if (intervalValue >= 5000) {
          settings.value.liveMatch.updateInterval = intervalValue
        }
      }
    ...
    if (saved) {
      try {
        ...
        settings.value = {
          ...DEFAULT_SETTINGS,
          ...parsed,
          // Ensure cards array exists and has default structure
